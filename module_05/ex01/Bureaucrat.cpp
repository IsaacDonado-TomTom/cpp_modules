...
void    Bureaucrat::signForm(Form& form) const
{
    ...
    if (form.getGradeToSign() < 1)
        str = " because grade to sign form is unreasonably high, get a new form.";
    else if (form.getGradeToSign() > 150)
        str = " because grade to sign form is unreasonably low, get a new form.";
    else
        str = " due to bureacrat grade being too low.";
    
    try
    {
        form.beSigned(*this);
        std::cout << "Bureaucrat[" << this->_name << "] has successfully signed form[" << form.getName() << "]" << std::endl;
    }
    catch(const std::exception& e)
    {
        std::cerr << e.what() << '\n' << "Bureaucrat[" << this->_name << "] was unable to sign[" << form.getName() << "]" << str << std::endl;;
    }
}
